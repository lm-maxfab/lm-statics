...
async function build () {
  ...
  for (const buildData of config.builds) {
    ...
    const files = await deepLs(BUILD)
    await batchFileEdit(
      files,
      BUILD,
      async fileData => {
        ...
        return content
          .replace(new RegExp(config.ROOT_URL_TEMPLATE, 'gm'), buildData.ROOT_URL)
          .replace(new RegExp(config.THIS_URL_TEMPLATE, 'gm'), `${buildData.ROOT_URL}/${relPath}`)
          .replace(new RegExp(config.PARENT_URL_TEMPLATE, 'gm'), `${buildData.ROOT_URL}/${path.join(relPath, '..')}`)
      },
      async fileData => {
        ...
        if (fileData.extension === '.scss') {
          ...
          await fse.writeFile(outPath, compiled, 'utf-8')
          await fse.rm(fileData.path)
        }
      }
    )
